...
import { ADACodesService } from "./adaCodesService";
...
export class SolidService {
  ...
  public async solidLogIn(podProvider: string, req: any, res: any) {
    ...
    let restApiport = process.env.RESTAPI_PORT || "5444";

    let clientName = "clinicaDental";
  ...
  public async successfulLogin(req: any, res: any): Promise<any> {
    let host = process.env.DOMAIN || "localhost";
    let restApiport = process.env.RESTAPI_PORT || "5444";
    let webappPort = process.env.WEBAPP_PORT || "5443";
  ...
  public async exportDataToPod(
    ...
    let interventions: Intervention[] =
      await this.interventionService.getInterventions(filter);

    for (let intervention of interventions) {
      let thingBuilder = EntityToRDF.interventionToRDF(intervention);

      for (let tooth of intervention.getTeeth()) {
        let toothThingBuilder = EntityToRDF.toothToRDF(tooth);
        let coding = this.getCoding(tooth).build();
      ...
      let codeThingBuilder = EntityToRDF.getCodeableConcept(
        ...
        intervention.getCode()
      );

      let codeCodingThingBuilder = EntityToRDF.getCoding(
        "https://cdtdentalcodes.com/",
        await ADACodesService.getAdaCode(intervention.getCode()),
        intervention.getName()
      ).build();
  ...
  private getCoding(code: string) {
    return EntityToRDF.getCodingForTooth(
